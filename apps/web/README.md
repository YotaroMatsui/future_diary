...
- 振り返り画面で自己モデル（SSOT）を編集・保存し、生成プロンプト全体はドロワーで確認/調整する。
...
- 生成プロンプトの全体確認・低頻度調整は振り返りページのドロワーに集約し、通常入力UIを簡潔に保つ。
