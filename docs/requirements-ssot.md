...
- 外部情報（天気/ニュース/他サービスの予定）を**ユーザ同意なしに**取り込んで断定的に予測すること
...
### 6.6 Google Calendar 連携（任意）

- ユーザが明示的に連携ボタンを押した場合のみ、Google Calendar OAuth を開始する
- 連携済みユーザの当日予定（primary calendar）を取得し、未来日記生成コンテキストに注入する
- 予定は断定情報としてではなく「当日の計画メモ」として扱い、編集可能な下書き体験を優先する
...
- 振り返りページで以下を編集・保存できる：
  - 日記の目的
  - 日記の特徴（筆致）
- 生成に使われるプロンプト全体をドロワーで確認でき、低頻度の調整項目（文頭/文末/段落数など）を編集できる
- ドロワーには「プロンプト初期化」操作を置き、シンプルな振り返り向け初期値へ戻せる
- 振り返りページで以下を表示する（自動抽出・非編集）：
  - 日々の実践蓄積から得られたナレッジ要約
- 上記自己モデルは未来日記生成プロンプトのSSOTとして扱う
- 保存済み日記から軽量分析を行い、振り返り項目の初期値候補を提示する（ユーザ編集を優先）
...
6. 過去の日記を日付から閲覧できる
7. 振り返りページで自己モデルを編集・保存できる
8. 未来日記生成時に、保存済み自己モデルがプロンプト注入のSSOTとして利用される
